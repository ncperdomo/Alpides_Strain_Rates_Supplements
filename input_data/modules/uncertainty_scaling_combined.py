...
def plot_uncertainty_distributions(original_uncertainties, scaled_uncertainties, component, figures_path, save_fig=True):
    """
    ...
    if save_fig:
        output_figure_path = os.path.join(figures_path, f'Alpides_{filename}')
...
def harmonise_uncertainties(input_folder, reference_filename, output_folder, figures_path, save_fig=True):
    ...
    for solution_file in os.listdir(input_folder):
        if solution_file.endswith('.csv'):
            ...
            print(f"Harmonising velocity uncertainties for {solution_name} dataset...")
            ...
            for component in ['E.sig', 'N.sig']:
                ...
                # Here I plot the uncertainty distributions only for the East and North components
                # Plotting uncertainty distributions (original raw vs scaled) with lognormal fit, mean lines, and 99th percentile
                plot_uncertainty_distributions(raw_uncertainties, scaled_uncertainties, component, figures_path, save_fig=save_fig)
            ...
            output_file_path = os.path.join(output_folder, f'Alpides_{solution_name}_scaled.csv')
            solution_df.to_csv(output_file_path, index=False, sep='\t')

def plot_lognormal_fit(original_uncertainties, component, figures_path, save_fig=True):
    """
    Plot the original and scaled uncertainty distributions, lognormal fit, 99th percentile, and mean lines.
    """
    # Ensure we only fit the lognormal on positive values
    ...
    if component == 'E.sig':
        component_string = '(East)'
        plt.hist(original_uncertainties, bins=40, alpha=1, label=f'Vel. uncertainties {component_string}', density=False, color='wheat')
    elif component == 'N.sig':
        component_string = '(North)'
        plt.hist(original_uncertainties, bins=40, alpha=1, label=f'Vel. uncertainties {component_string}', density=False, color='lightsteelblue')

    # Plot the fitted lognormal curve (scaled to match the counts)
    count, bins, _ = plt.hist(positive_uncertainties, bins=40, alpha=0.0)  # Get the bin heights for raw data
    scale_factor = max(count) / max(pdf_vals)  # Scale factor to align the PDF to the counts
    plt.plot(x_vals, pdf_vals * scale_factor, label='Lognormal fit', color='red', linewidth=2)
    
    # Plot the 99th percentile vertical dashed line for the original uncertainties
    plt.axvline(p99, color='gray', linestyle='dashed', linewidth=2, label=f'99th percentile: {p99:.2f}')
    
    # Plot vertical dashed lines for the mean of the raw and scaled uncertainties
    plt.axvline(raw_mean, color='blue', linestyle='dashed', linewidth=2, label=f'Mean: {raw_mean:.2f}')
    ...
    # Save the figures to the results folder
    if save_fig:
        output_figure_path = os.path.join(figures_path, f'Alpides_example_{filename}')
        plt.savefig(output_figure_path, format='pdf', dpi=300)
    plt.show()