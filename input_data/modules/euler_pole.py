# This module was retrieved from the following repository: https://github.com/joferkington/euler_pole
# The module was created by Joe Kington and is licensed under the MIT License. 
...
def cart2sph(x,y,z):
    ...
    # Check if r is close to machine precision
    if r < np.finfo(float).eps:
        r += np.finfo(float).eps  # Add a small tolerance to avoid division by zero
    lat = np.arcsin(z / r)
    lon = np.arctan2(y, x)
